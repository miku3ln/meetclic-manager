...
class ClinicalExamController extends MyBaseController
{
     ...
     */
    public $table = "clinical_exam";
    public $name_manager = "Examenes Clinicos Tipos";

    public function index()
    {
        $model = new ClinicalExam();
...
//Change
    public function getListClinicalExams()
    {
        $data = Request::all();
        $query = ClinicalExam::query();
    ...
    public function getFormClinicalExam($id = null)
    {
        $method = 'POST';
        $model = isset($id) ? ClinicalExam::find($id) : new ClinicalExam();
        $camerCase = $model->getCamelCase();
    ...
    {
        $data = Request::all();
        $query = ClinicalExam::query()->select('id', 'name as text');
        ...
        $query->where('status', '=', ClinicalExam::STATUS_ACTIVE);
    ...
    public function postSave()
    {
        ...
        try {
            ...
            if ($attributesPost[$model->getTable() . '_id'] == '') { //Create
                ...
                $model = ClinicalExam::find($attributesPost[$model->getTable() . '_id']);
                if (isset($data['status']))
                    $model->status = $attributesPost['status'];
            }
            $model->name = trim($attributesPost['name']);
            $model->description = trim($attributesPost['description']);
            $model->save();
