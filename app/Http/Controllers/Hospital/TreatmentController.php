...
class TreatmentController extends MyBaseController
{
    ...
    public $name_manager = "Prestaciones-Tratamiento";
...
//Change
    public function getListTreatments()
    {
        $data = Request::all();
    ...
    public function getFormTreatment($id = null)
    {
        $method = 'POST';
        $model = isset($id) ? Treatment::find($id) : new Treatment();
        $camerCase = $model->getCamelCase();
    ...
    {
        $data = Request::all();
        $query = Treatment::query()->select('id', 'name as text');
        ...
        $query->where('status', '=', Treatment::STATUS_ACTIVE);
    ...
    public function postSave()
    {
        ...
        try {
            ...
            if ($attributesPost[$model->getTable() . '_id'] == '') { //Create
                ...
                $model = Treatment::find($attributesPost[$model->getTable() . '_id']);
