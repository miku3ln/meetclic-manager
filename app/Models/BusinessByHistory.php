...
class BusinessByHistory extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'business_by_history';
    ...
    protected $attributesData = [
        ...
        ['column' => 'author', 'type' => 'string', 'defaultValue' => '', 'required' => 'true'],
        ['column' => 'author_titles', 'type' => 'string', 'defaultValue' => '', 'required' => 'true'],
    ...
    public function getAdmin($params)
    {
        ...
        $selectString = "$this->table.id,$this->table.value,$this->table.description,$this->table.status,$this->table.author,$this->table.author_titles,business.title as business,
business.id as business_id,
$this->table.source,$this->table.allow_source,$this->table.subtitle,$this->table.main";
        ...
        if ($params['searchPhrase'] != null) {
            ...
            $query->orWhere($this->table . '.author', 'like', '%' . $likeSet . '%');
            $query->orWhere($this->table . '.author_titles', 'like', '%' . $likeSet . '%');
    ...
    public function saveData($params)
    {
        ...
        try {
            ...
            if (isset($attributesPost["id"]) && $attributesPost["id"] != "null" && $attributesPost["id"] != "-1") {
                $model = BusinessByHistory::find($attributesPost['id']);
                $createUpdate = false;
            ...
            $BusinessByHistoryData = $attributesPost;
            ...
            if ($allow_source == 1) {
                ...
                $pathSet = "/uploads/web/businessByHistory/images";
            ...
            if ($successMultimedia) {
                ...
                $source = $allow_source == 1 ? $currentResource . $successMultimediaModel['source'] : $BusinessByHistoryData['source'];


                $BusinessByHistoryData['source'] = $source;

                $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $BusinessByHistoryData, 'attributesData' => $this->attributesData));
                ...
                if ($success) {
                    if ($attributesSet['main'] == 1) {
                        if (isset($attributesPost["id"]) && $attributesPost["id"] != "null" && $attributesPost["id"] != "-1") {
                            ...
                            BusinessByHistory::where('status', 'ACTIVE')
                    ...
                    $success = false;
                    $msj = "Problemas al guardar  BusinessByHistory.";
    ...
    public function getMainFrontend($params)
    {
        $sort = 'desc';
        $field = 'main';
...
$this->table.source,$this->table.allow_source,$this->table.subtitle,$this->table.main";
    ...
    public function getManagementFrontend($params)
    {
        ...
        if ($resultData) {
            ...
            $success = true;
            $business_by_history_id = $resultData->id;
            $modelData = new \App\Models\BusinessHistoryByData();
            $childrens =    $modelData->getDataById([
                'filters' => [
                    'business_by_history_id' => $business_by_history_id
                ]
            ]);
            if ($childrens) {
