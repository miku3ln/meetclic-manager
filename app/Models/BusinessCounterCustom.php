...
class BusinessCounterCustom extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'business_counter_custom';
...
business.id as business_id";
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'BusinessCounterCustom';
            $model = new BusinessCounterCustom();
            $createUpdate = true;

            if (isset($attributesPost["id"]) && $attributesPost["id"] != "null" && $attributesPost["id"] != "-1") {
                $model = BusinessCounterCustom::find($attributesPost['id']);
                $createUpdate = false;
            ...
            $BusinessCounterCustomData = $attributesPost;
            ...
            if ($successMultimedia) {
                ...
                $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $BusinessCounterCustomData, 'attributesData' => $this->attributesData));
                ...
                if ($success) {
                    ...
                    $success = false;
                    $msj = "Problemas al guardar  BusinessCounterCustom.";
    ...
    public function getDataManagementFrontend($params)
    {
        $dataCurrent = $this->getDataFrontend($params);
        $success = false;
        $data = [];

        if ($dataCurrent) {
            $business_counter_custom_id = $dataCurrent->id;
            $modelData = new \App\Models\BusinessCounterCustomByData();
            $infoChildrens =   $modelData->getDataCounterKey([
                'filters'=>['business_counter_custom_id' => $business_counter_custom_id]
