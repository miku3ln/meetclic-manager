...
class CashByMovement extends  ModelManager
{
...
['column'=>'cash_id','type'=>'integer','defaultValue'=>'','required'=>'true'],
['column'=>'movement_type','type'=>'integer','defaultValue'=>'','required'=>'true'],
['column'=>'cash_reason_id','type'=>'integer','defaultValue'=>'','required'=>'true'],
...
"cash_reason_id"=>"required|numeric",
...
$selectString = "$this->table.id,$this->table.user_id,$this->table.state,$this->table.cash_id,$this->table.movement_type,$this->table.cash_reason_id,$this->table.accounting_account_id,$this->table.details,$this->table.rode,$this->table.date_current,$this->table.transaction_type,$this->table.entity_type,$this->table.entity_id,$this->table.created_at,$this->table.update_at,$this->table.available_balance";
...
$query->orWhere($this->table.'.cash_id','like','%'.$likeSet.'%');
$query->orWhere($this->table.'.movement_type','like','%'.$likeSet.'%');
$query->orWhere($this->table.'.cash_reason_id','like','%'.$likeSet.'%');
...
DB::beginTransaction();
try {
$modelName= 'CashByMovement';
$model = new CashByMovement();
$createUpdate = true;

    if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
    $model = CashByMovement::find($attributesPost[$modelName]['id']);
    $createUpdate = false;
    } else {
    $createUpdate = true;
    }
    ...
    $cashByMovementData = $attributesPost[$modelName];
    $attributesSet = $this->getValuesModel(array('fillAble'=>$this->fillable,'haystack'=>$cashByMovementData,'attributesData'=>$this->attributesData));
    ...
    $success = false;
    $msj = "Problemas al guardar  CashByMovement.";
