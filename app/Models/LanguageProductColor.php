...
class LanguageProductColor extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'language_product_color';
...
product_measure_type.id as product_color_id
";

        $select = DB::raw($selectString);
        $query->select($select);
        $query->join('language', 'language.id', '=', $this->table . '.language_id');
        $query->join('product_measure_type', 'product_measure_type.id', '=', $this->table . '.product_color_id');
...
// Pagination: $perpage 0; get all data
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'LanguageProductColor';
            $model = new LanguageProductColor();
            $createUpdate = true;

            if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                $model = LanguageProductColor::find($attributesPost[$modelName]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            }
            ...
            $languageProductColorData = $attributesPost[$modelName];
            $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $languageProductColorData, 'attributesData' => $this->attributesData));
            ...
            $entity_manager_key = 'product_color_id';
            $entity_manager_id = $attributesPost[$modelName]['product_color_id'];
            ...
            if ($success) {
                ...
                if ($attributesSet['state'] == 'ACTIVE') {
                    ...
                    if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                        ...
                        LanguageProductColor::where('state', 'ACTIVE')
                ...
                $success = false;
                $msj = "Problemas al guardar  LanguageProductColor.";
    ...
    public function setDelete($params)
    {
        ...
        DB::beginTransaction();
        try {
            $model = LanguageProductColor::find($id);
            if ($model) {
