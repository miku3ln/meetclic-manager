...
use Auth;
class LanguageProductSubcategory extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'language_product_subcategory';
...
";
        ...
        $entity_name = 'product_subcategory_id';
...
// Pagination: $perpage 0; get all data
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'LanguageProductSubcategory';
            $model = new LanguageProductSubcategory();
            $createUpdate = true;

            if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                $model = LanguageProductSubcategory::find($attributesPost[$modelName]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            }
            ...
            $languageProductSubcategoryData = $attributesPost[$modelName];
            $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $languageProductSubcategoryData, 'attributesData' => $this->attributesData));
            ...
            $entity_manager_key = 'product_subcategory_id';
            $entity_manager_id = $attributesPost[$modelName]['product_subcategory_id'];
            ...
            if ($success) {
                ...
                if ($attributesSet['state'] == 'ACTIVE') {
                    if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                        ...
                        LanguageProductSubcategory::where('state', 'ACTIVE')
                ...
                $success = false;
                $msj = "Problemas al guardar  LanguageProductSubcategory.";
    ...
    public function setDelete($params)
    {
        ...
        DB::beginTransaction();
        try {
            $model = LanguageProductSubcategory::find($id);
            if ($model) {
