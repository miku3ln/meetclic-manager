...
class LanguageTemplateSliderByImages extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'language_template_slider_by_images';
    ...
    protected $attributesData = [
        ...
        ['column' => 'template_slider_by_images_id', 'type' => 'integer', 'defaultValue' => '', 'required' => 'true']
    ...
    public static function getRulesModel()
    {
        $rules = ["title" => "required|max:200",
            ...
            "template_slider_by_images_id" => "required|numeric"
        ];
        return $rules;
    }
    ...
    public function getAdminData($params)
    {
        ...
        $selectString = "$this->table.id,$this->table.title,$this->table.state,$this->table.subtitle,language.value as language,
language.id as language_id,
template_slider_by_images.title as template_slider_by_images,
template_slider_by_images.id as template_slider_by_images_id
";

        $select = DB::raw($selectString);
        $query->select($select);
        $query->join('language', 'language.id', '=', $this->table . '.language_id');
        $query->join('template_slider_by_images', 'template_slider_by_images.id', '=', $this->table . '.template_slider_by_images_id');
        if ($params['searchPhrase'] != null) {
            ...
            $query->orWhere("template_slider_by_images.title", 'like', '%' . $likeSet . '%');;

        }
        $entity_name = 'template_slider_by_images_id';
...
// Pagination: $perpage 0; get all data
    ...
    public function saveDataManager($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'LanguageTemplateSliderByImages';
            $model = new LanguageTemplateSliderByImages();
            $createUpdate = true;

            if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                $model = LanguageTemplateSliderByImages::find($attributesPost[$modelName]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            }
            ...
            $entity_manager_key = 'template_slider_by_images_id';
            $entity_manager_id = $attributesPost[$modelName]['template_slider_by_images_id'];
            ...
            if ($success) {
                ...
                if ($attributesSet['state'] == 'ACTIVE') {
                    if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                        ...
                        LanguageTemplateSliderByImages::where('state', 'ACTIVE')
                ...
                $success = false;
                $msj = "Problemas al guardar  LanguageTemplateSliderByImages.";
    ...
    public function getListSelect2($params)
    {
        ...
        $query->select($select);
        $query->join('language', 'language.id', '=', $this->table . '.language_id');
        $query->join('template_slider_by_images', 'template_slider_by_images.id', '=', $this->table . '.template_slider_by_images');
        if (isset($params["filters"]['search_value']["term"])) {
            ...
            $query->orWhere($this->table . '.state', 'like', '%' . $likeSet . '%');
            $query->orWhere($this->table . '.subtitle', 'like', '%' . $likeSet . '%');
            $query->orWhere("language.", 'like', '%' . $likeSet . '%');
            $query->orWhere("template_slider_by_images.", 'like', '%' . $likeSet . '%');;
    ...
    public function setDeleteManager($params)
    {
        $success = false;
        $msj = "";
        $result = array();
        ...
        DB::beginTransaction();
        try {
            $model = LanguageTemplateSliderByImages::find($id);
            if ($model) {
    ...
    {
       return $this->setDeleteManager($params);
    }
    public function setDeleteActivitiesGamification($params)
    {
        return $this->setDeleteManager($params);
    }
    public function setDeleteRewardsGamification($params)
    {
        return $this->setDeleteManager($params);
    }
}
