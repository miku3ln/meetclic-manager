...
class ProjectHeaderByResources extends ModelManager
{


    protected $table = 'project_header_by_resources';
    protected $nameModel = 'ProjectHeaderByResources';
    ...
    public function saveData($params)
    {
        ...
        try {
            ...
            if (isset($attributesPost[$this->nameModel]["id"]) && $attributesPost[$this->nameModel]["id"] != "null" && $attributesPost[$this->nameModel]["id"] != "-1") {
                $model = ProjectHeaderByResources::find($attributesPost[$this->nameModel]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            ...
            if ($type_multimedia == ProjectHeaderByResources::TYPE_IMAGE_UPLOAD) {
                $type = 'image';
                $allowUpload = true;
            } else if ($type_multimedia == ProjectHeaderByResources::TYPE_URL_OTHER_SERVER) {
                $url = $urlManager;
                $allowUpload = false;
                $type = 'none';


            } else if ($type_multimedia == ProjectHeaderByResources::TYPE_FILE) {
                $url = $urlManager;
                $allowUpload = true;
                $type = 'docs';
            ...
            if ($allowUpload) {
                ...
                $source = $postData["url"];
                $pathSet = "/uploads/projects/ProjectHeaderByResources";
