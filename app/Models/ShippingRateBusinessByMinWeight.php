...
class ShippingRateBusinessByMinWeight extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'shipping_rate_business_by_min_weight';
...
// Pagination: $perpage 0; get all data
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'ShippingRateBusinessByMinWeight';
            $model = new ShippingRateBusinessByMinWeight();
            $createUpdate = true;

            if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                $model = ShippingRateBusinessByMinWeight::find($attributesPost[$modelName]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            }
            ...
            $shippingRateBusinessByMinWeightData = $attributesPost[$modelName];
            $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $shippingRateBusinessByMinWeightData, 'attributesData' => $this->attributesData));
            ...
            if ($success) {
                ...
                $success = false;
                $msj = "Problemas al guardar  ShippingRateBusinessByMinWeight.";
