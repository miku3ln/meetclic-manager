...
class TemplateNewsByData extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'template_news_by_data';
    ...
    protected $attributesData = [
        ...
        ['column' => 'template_news_id', 'type' => 'integer', 'defaultValue' => '', 'required' => 'true'],
    ...
    public static function getRulesModel()
    {
        $rules = ["title" => "required",
            ...
            "template_news_id" => "required|numeric",
            "title_icon" => "max:15"
        ];
        return $rules;
    }
    ...
    public function getAdmin($params)
    {
        ...
        $template_news_id = $params['filters']['template_news_id'];
        ...
        $selectString = "$this->table.id,$this->table.title,$this->table.description,$this->table.status,$this->table.source,$this->table.allow_source,template_news.value as template_news,
template_news.id as template_news_id,
$this->table.title_icon";

        $select = DB::raw($selectString);
        $query->select($select);
        $query->join('template_news', 'template_news.id', '=', $this->table . '.template_news_id');
        if ($params['searchPhrase'] != null) {
            ...
            $query->orWhere("template_news.value", 'like', '%' . $likeSet . '%');
            $query->orWhere($this->table . '.title_icon', 'like', '%' . $likeSet . '%');

        }
        $query->where($this->table . '.template_news_id', '=', $template_news_id);
...
// Pagination: $perpage 0; get all data
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'TemplateNewsByData';
            $model = new TemplateNewsByData();
            $createUpdate = true;
            $auxResource = "";

            if (isset($attributesPost["id"]) && $attributesPost["id"] != "null" && $attributesPost["id"] != "-1") {
                $model = TemplateNewsByData::find($attributesPost['id']);
                $createUpdate = false;
            ...
            $templateNewsByDataData = $attributesPost;


            $modelMultimedia = new Multimedia;
            $template_news_id = $attributesPost["template_news_id"];
            ...
            if ($successMultimedia) {
                ...
                $source = $allow_source == 1 ? $currentResource . $successMultimediaModel['source'] : $templateNewsByDataData['source'];

                $templateNewsByDataData['source'] = $source;

                $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $templateNewsByDataData, 'attributesData' => $this->attributesData));
                ...
                if ($success) {
                    ...
                    $success = false;
                    $msj = "Problemas al guardar  TemplateNewsByData.";
    ...
    public function getServiceDataFrontend($params)
    {
        ...
        $relationLanguage = 'language_template_news_by_data';
        $relationLanguageId = 'template_news_by_data_id';
