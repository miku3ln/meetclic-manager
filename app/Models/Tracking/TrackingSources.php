...
class TrackingSources extends ModelManager
{
    protected $table = 'tracking_sources';
    ...
    {
        return [
            'uid' => 'required|string|max:32|unique:tracking_sources,uid',
            'code' => 'required|string|max:32|unique:tracking_sources,code',
    ...
    public function saveData($params)
    {
        ...
        try {
            $modelName = 'TrackingSource';
            $data = $params['attributesPost'][$modelName];
            $createUpdate = true;
            $model = new TrackingSource();

            if (!empty($data['id']) && $data['id'] !== "null" && $data['id'] !== "-1") {
                $model = TrackingSource::find($data['id']);
                $createUpdate = false;
            }
            ...
            if ($validateResult['success']) {
                ...
                $success = false;
                $msj = "Error al guardar TrackingSource.";
