...
class TreatmentByDetails extends ModelManager
{
    const STATE_ACTIVE = 'ACTIVE';
    const STATE_INACTIVE = 'INACTIVE';
    protected $table = 'treatment_by_details';
    ...
    public function getAdmin($params)
    {
        ...
        $selectString = "$this->table.id,$this->table.product_id,$this->table.quantity,$this->table.quantity_unit,$this->table.discount_percentage,$this->table.discount_percentage_unit,$this->table.discount_value,$this->table.discount_value_unit,$this->table.unit_price,$this->table.unit_price_unit,$this->table.management_type,$this->table.tax_percentage,$this->table.subtotal,$this->table.total,$this->table.description,$this->table.product_type,treatment_by_patient.invoice_code as treatment_by_patient,
...
// Pagination: $perpage 0; get all data
    ...
    public function saveData($params)
    {
        ...
        DB::beginTransaction();
        try {
            $modelName = 'TreatmentByDetails';
            $model = new TreatmentByDetails();
            $createUpdate = true;

            if (isset($attributesPost[$modelName]["id"]) && $attributesPost[$modelName]["id"] != "null" && $attributesPost[$modelName]["id"] != "-1") {
                $model = TreatmentByDetails::find($attributesPost[$modelName]['id']);
                $createUpdate = false;
            } else {
                $createUpdate = true;
            }
            ...
            $treatmentByDetailsData = $attributesPost[$modelName];
            $attributesSet = $this->getValuesModel(array('fillAble' => $this->fillable, 'haystack' => $treatmentByDetailsData, 'attributesData' => $this->attributesData));
            ...
            if ($success) {
                ...
                $success = false;
                $msj = "Problemas al guardar  TreatmentByDetails.";
    ...
    {
        $treatment_by_patient_id = $params['treatment_by_patient_id'];
        $query = DB::table($this->table);
        $selectString = "$this->table.id,$this->table.product_id,$this->table.quantity,$this->table.quantity_unit,$this->table.discount_percentage,$this->table.discount_percentage_unit,$this->table.discount_value,$this->table.discount_value_unit,$this->table.unit_price,$this->table.unit_price_unit,$this->table.management_type,$this->table.tax_percentage,$this->table.subtotal,$this->table.total,$this->table.description,$this->table.product_type,treatment_by_patient.invoice_code as treatment_by_patient,
treatment_by_patient.id as treatment_by_patient_id
,product.code product_code,product.name product_name,product.description product_description
";

        $select = DB::raw($selectString);
        $query->select($select);
        $query->join('treatment_by_patient', 'treatment_by_patient.id', '=', $this->table . '.treatment_by_patient_id');
        $query->join('product', 'treatment_by_patient.id', '=', $this->table . '.treatment_by_patient_id');
