(function ($) {
    ...
    $(document).keyup(function (event) {
        ...
        if (event.which == '27') {
            ...
            if ($('.active-body-search-overlay').length) {
                _closeSearch();
            } else {
    ...
    function closeNoAnimation(id, image, finalWidth, maxQuickWidth) {
        ...
        if (image.length) {
            parentListItem = image.parent('.image-wrap'),
                topSelected = image.offset().top - $(window).scrollTop(),
                leftSelected = image.offset().left,
                widthSelected = image.width();
        }
    ...
    $('.pro-qty').append('<a  class="inc qty-btn"><i class="pe-7s-plus"></i></a>');
    $('.pro-qty').prepend('<a  class= "dec qty-btn"><i class="pe-7s-less"></i></a>');
...
function processFormSearch(e) {
    if (e.preventDefault) e.preventDefault();
    var currentUrl = this.action;
...
function initFormSearch(){
    var form = document.getElementById('search-data-form');
    console.log('input t');
    if (form) {

        if (form.attachEvent) {
            form.attachEvent("submit", processFormSearch);
        } else {
