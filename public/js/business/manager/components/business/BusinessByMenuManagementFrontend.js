var componentThisBusinessByMenuManagementFrontend;

Vue.component('business-by-menu-management-frontend-component', {

    template: '#business-by-menu-management-frontend-template',
    directives: {
        initS2: {
    ...
    mounted: function () {
        componentThisBusinessByMenuManagementFrontend = this;
    ...
    validations: function () {
        ...
        if (!this.model.attributes.type_item) {//has children

            if (this.model.attributes.type == 0) {

            }
            else if (this.model.attributes.type == 1) {//root
                attributes['link'] = {};
            }
        } else {//not has

            if (this.model.attributes.type == 1) {//is parent main

            } else if (this.model.attributes.type == 2) {
                attributes['parent_id_data'] = {required};

            } else if (this.model.attributes.type == 3) {//level 3
                attributes['parent_id_data'] = {required};
...
//**Modal*
            ...
            tabCurrentSelector: '#tab-business-by-menu-management-frontend',
            processName: "Registro Acción.",
            formConfig: {
                nameSelector: "#business-by-menu-management-frontend-form",
                url: $('#action-business-by-menu-management-frontend-save').val(),
                loadingMessage: 'Guardando...',
                errorMessage: 'Error al guardar la acción.',
                successMessage: 'La Acción se guardo correctamente.',
                nameModel: "BusinessByMenuManagementFrontend"
            },
            gridConfig: {
                selectorCurrent: "#business-by-menu-management-frontend-grid",
                url: $("#action-business-by-menu-management-frontend-admin").val()
            },
            submitStatus: "no",
            showManager: false,
            businessId: null,
    ...
    methods: {
            ...
            gridInit.bootgrid({
                ...
                formatters: {
                    'description': function (column, row) {
                        ...
                        var linkHtml = (row.type == 2 || row.type == 3 || row.type == 0) ? [
                            "<a href='" + row.link + "' target='_blank'>",
                            "Link Gestión",
                            "</a>",
                        ] : [];
                        linkHtml = linkHtml.join("");
                        var linkDescription = (row.type == 2 || row.type == 3 || row.type == 0) ? [
                        ...
                        var typeLabel = row.type == 0 ? "Principal" : (row.type == 1 ? "Menu Principal" : (row.type == 2 ? "Segundo Nivel" : "Tercer Nivel"));
        ...
        _managerRowGrid: function (params) {
            ...
            if (params.managerType == "updateEntity") {
                ...
                if (this.model.attributes.type_item) {
                    dataTypes = [
                        {id: 1, text: "Menu Principal"},
                        {id: 2, text: "Segundo Nivel"},
                        {id: 3, text: "Tercer Nivel"},
        ...
        _typeItem: function (type) {
            ...
            var dataTypes = [];
            this.$v.model.attributes["parent_id_data"].$model = null;
            this.$v.model.attributes["parent_id_data"].$reset();


            if (type) {
                this.$v.model.attributes["type"].$model = 1;
                dataTypes = [
                    {id: 1, text: "Menu Principal"},
                    {id: 2, text: "Segundo Nivel"},
                    {id: 3, text: "Tercer Nivel"},
...
//Manager Model
        getValuesSave: function () {
            var business_id = this.businessId;
            var result = {
                BusinessByMenuManagementFrontend: {
        ...
        _managerS2Action: function (params) {
            ...
            function getTypeManagement(params) {
                var type = $scope.model.attributes.type;
                var business_id = $scope.businessId;

                var paramsFilters = {filters: {search_value: params.term, type: type, business_id: business_id}};
            ...
            var elementInit = $(el).select2({
                ...
                ajax: {// instead of writing the function to execute the request we use Select2's convenient helper
                    url: $("#action-business-by-menu-management-frontend-listActionsParent").val(),
                    type: "get",
                    dataType: 'json',
                    data: function (term, page) {
                        var paramsFilters = getTypeManagement({
                            term: term, page: page
                        });
