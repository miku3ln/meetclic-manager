let configProcess = {//CPP-004
    'manager': UtilManagerCustomModel.getManagerData('manager_dashboard'),
    'component': null,
};
//TODO UPLOAD
...
function buildAllCharts(data) {
    // 1. Location
    const locGroups = {};
    const locCategories = [...new Set(data.dataCompanyInteractionsByLocation.map(i => `${i.city} - ${i.country}`))];
    data.dataCompanyInteractionsByLocation.forEach(i => {
        const loc = `${i.city} - ${i.country}`;
        if (!locGroups[i.companyName]) locGroups[i.companyName] = {};
        locGroups[i.companyName][loc] = i.totalInteractions;
    });
    const locSeries = Object.keys(locGroups).map(name => ({
        name,
        data: locCategories.map(loc => locGroups[name][loc] || 0)
    }));
    Highcharts.chart('chart-location', {
        chart: {type: 'column'},
        xAxis: {categories: locCategories},
        yAxis: {title: {text: 'Interacciones'}},
        series: locSeries
    });

    // 2. Traffic Sources
    const srcGroups = {};
    const srcCategories = [...new Set(data.dataCompanyTrafficSources.map(i => i.sourceCode))];
    data.dataCompanyTrafficSources.forEach(i => {
        if (!srcGroups[i.companyName]) srcGroups[i.companyName] = {};
        srcGroups[i.companyName][i.sourceCode] = i.totalVisits;
    });
    const srcSeries = Object.keys(srcGroups).map(name => ({
        name,
        data: srcCategories.map(code => srcGroups[name][code] || 0)
    }));
    Highcharts.chart('chart-sources', {
        chart: {type: 'bar'},
        xAxis: {categories: srcCategories},
    ...
    const dayGroups = {};
    const dates = [...new Set(data.dataDailyInteractionsByCompanyType.map(i => i.date))];
    data.dataDailyInteractionsByCompanyType.forEach(i => {
        if (!dayGroups[i.companyType]) dayGroups[i.companyType] = {};
        dayGroups[i.companyType][i.date] = i.totalInteractions;
    });
    const dailySeries = Object.keys(dayGroups).map(type => ({
        name: type,
    ...
    const usersSeries = data.dataUserParticipationByCompany.map(i => ({
        name: `${i.userType} - ${i.companyName}`,
        y: i.totalSessions
    }));
    Highcharts.chart('chart-users', {
        chart: {type: 'pie'},
        series: [{name: 'Sesiones', data: usersSeries}]
    });

    // 5. Click types
    const clickGroups = {};
    const clickTypes = [...new Set(data.dataClickTypesByCompany.map(i => i.clickType))];
    data.dataClickTypesByCompany.forEach(i => {
        if (!clickGroups[i.companyName]) clickGroups[i.companyName] = {};
        clickGroups[i.companyName][i.clickType] = i.totalClicks;
    });
    const clickSeries = Object.keys(clickGroups).map(name => ({
        name,
...
$(function () {
    ...
    $("#applyFilters").on("click", function () {
        ...
        fetchAndRenderCharts(startDate, endDate);
    });
});

function fetchAndRenderCharts(startDate, endDate) {
    $.ajax({
        url: $("#action_getDataInteraction").val(),
        method: 'GET',
        data: {
...
Vue.component(configProcess.manager.modelProcess + '-component', {
    ...
    data: function () {
        ...
        let dataManager = {
            ...
            managerTracking: {
                web: {
                    title: "Tracking Página Web",
                },
                business: {
                    title: "Tracking Business Links ",
                }
            },

            processName: "Registro de Planificacion orueb.",
            formConfig: {
    ...
    methods: {
        ...
        initCurrentComponent: function () {
            ...
            Highcharts.chart('container-annotation', {
                chart: {
                    type: 'area',
                    zooming: {
                        type: 'x'
            ...
            Highcharts.chart('container-line-labels', {
                chart: {
                    type: 'line'
                },
                title: {
            ...
            const chart = Highcharts.chart('container-are-race-chart', {
                chart: {
                    events: {
