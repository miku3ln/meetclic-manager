    ...
    $(".add-cart,.single-icon--add-to-cart").on('click', function () {
        ...
        if ($(this).hasClass('add-cart--product-details')) {
            ...
            if ($(this).hasClass('add-cart--shop-preview')) {
                ...
                product = rowData[0];
            } else {//ADD CART ROW -MANAGER GRID
        ...
        if (allowPush) {
            ...
            if (typeManagement == null) {
                ...
                var selectorCurrent = '#row-outlet-' + +product['id'] + ' span.single-icon--quick-view a.cd-trigger--manager-quick-view-home';

                $(selectorCurrent).click();
            } else if (typeManagement == 5) {
                var selectorCurrent = '#row-balance-' + product['id'] + ' span.single-icon--quick-view a.cd-trigger--manager-quick-view-home';
                $(selectorCurrent).click();
            }
        }
        if (allowViewPush) {
...
function getViewsRowProduct($params) {
    ...
    var shoppingButton = $allowShop == 1 ? ['             <span class="single-icon single-icon--add-to-cart"  product-id="' + data.id + '">',
    ...
    var viewButtonProduct = $allowShop == 1 ?
        [
            ...
            '              <span class="single-icon single-icon--quick-view single-icon--quick-view--not-basket" row-id="row-' + data.id + '"> ',
            '                    <a  id="row-' + data.id + '" class="cd-trigger cd-trigger--manager-quick-view" href="#qv-1" data-tippy="Quick View" ',
            '                        data-tippy-inertia="true"',
            '                        data-tippy-animation="shift-away"',
            '                       data-tippy-delay="50" ',
