...
/*---MANAGEMENT--*/
var saveDataOdontogram;
var initCara = false;
var typeDPBO = "PERMANENT";
/*--Content Parent ODONTOGRAM---*/
var contentParentMeasure = "#m_portlet_admin_odontogram_by_patient";

function renderOdontograms() {
    ...
    var functionsObjThis = {
        initOdontogramsSvg: function (callback = null) {
            /* if (!initOdontogram) {*/
            var result = [];
            var url_svg = "svg/odontograma/odontograma_inferior.svg";
            ...
            Snap.load(url_svg, function (f) {
                ...
                var url_svg = "svg/odontograma/odontograma_superior.svg";
    ...
    function viewFrm(initCara) {

        if (initCara) {
            if (OdontogramByPatient) {
                OdontogramByPatient.initDentalPieceByOdontogramFrm(saveDataOdontogram);

            } else if (TreatmentPlanByPatientForm) {
                TreatmentPlanByPatientForm.initDentalPieceByOdontogramFrm(saveDataOdontogram);

            }
            $(contentElementFrm).show();
        }
        else {
    ...
    function setDimention(elementSNAPSVGChange, params = null) {
        var width = $(contentParentMeasure).width() - $(contentParentMeasure).width() * 8 / 100;
        var height = params ? params.height : 200;
