var componentThisPeopleNationality;
Vue.component('people-nationality-component', {
    directives: {
        initS2: {
            inserted: function (el, binding, vnode, vm, arg) {
                var paramsInput = binding.value;
                paramsInput._managerS2Countries({
    ...
    template: '#peopleNationality-template',
    props: {
        params: {
            type: Object,
        }
    ...
    mounted: function () {
        componentThisPeopleNationality = this;
...
//**Modal*
            ...
            tabCurrentSelector: '#tab-peopleNationality',
            processName: "Registro Ruc.",
            formConfig: {
                nameSelector: "#peopleNationality-form",
                url: $('#action_peopleNationality_save').val(),
                loadingMessage: 'Guardando...',
                errorMessage: 'Error al guardar la nacionalidad.',
                successMessage: 'La nacionalidad se guardo correctamente.',
                nameModel: "PeopleNationality"
            },
            gridConfig: {
                selectorCurrent: "#peopleNationality-grid",
                url: $("#action_peopleNationality_admin").val()
            },
            submitStatus: "no",
            showManager: false,
            managerType: null,
    ...
    methods: {
                ...
                formatters: {
                    'description': function (column, row) {
                        ...
                        var description = (row.name !== "null" && row.name) ? [
                            ...
                            "   <span class='content-description__title'>País:</span><span class='content-description__value'>" + (row.country) + "</span>",
                            "</div>",
                            "<div class='content-description__information'>",
        ...
        _managerRowGrid: function (params) {
            ...
            if (params.managerType == "updateEntity") {
                ...
                this.model.attributes.countries_id_data = ({id: rowCurrent.countries_id, text: rowCurrent.country});
        ...
        /*FORM*/
        _countriesData: function (value) {

            this._setValueForm("countries_id_data", value);

        },
        _managerS2Countries: function (params) {
            ...
            var elementInit = $(el).select2({
                ...
                ajax: {// instead of writing the function to execute the request we use Select2's convenient helper
                    url: $("#action_peopleNationality_listS2Countries").val(),
                    type: "get",
                    dataType: 'json',
            ...
            elementInit.on('select2:select', function (e) {
                ...
                _this._setValueForm('countries_id_data', data);
        ...
        _setValueForm: function (name, value) {
            if (name == "countries_id_data") {
                this.$v["model"]["attributes"][name].$reset();
                if (value == null) {
                    this.$v["model"]["attributes"][name].$invalid = true;
                    this.$v["model"]["attributes"][name].$error = true;
                    this.$v["model"]["attributes"]["description"].$touch();
                } else {
                    this.$v["model"]["attributes"][name].$invalid = false;
                    this.$v["model"]["attributes"][name].$error = false;
        ...
        getClassErrorForm: function (nameElement, objValidate) {
            var resultError = this.viewErrorCurrent(nameElement);
            var result = {
                "form-group--error": resultError.error,
                'form-group--success': resultError.success
            };
            return result;
        },
        viewErrorCurrent: function (elementName) {
            var objValidate = this.$v.model.attributes[elementName];
            var modelValue = objValidate.$model;
            var result = {
                "error": objValidate.$error,
                'success': objValidate.$dirty ? (!objValidate.$error) : false
            };
            if (elementName == "countries_id_data") {
                result = {
                    "error": objValidate.$dirty ? (modelValue ? false : true) : false,
                    'success': objValidate.$dirty ? (!objValidate.$error) : false
                };
            }
            return result;
        },
        viewErrorCurrentCountry: function () {
            var result = this.viewErrorCurrent('countries_id_data');
