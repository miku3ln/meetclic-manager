...
function getFormRegister(action) {
    ajaxRequest(action, {
        ...
        success_callback: function (data) {
            ...
            initEvents();
            setTimeout(function () {
                setColorTypeReference();
            }, 1000)
        }
    });
}

function setColorTypeReference() {
    var params = {color: $('#reference_type_id option:selected').attr('color-current')};
    $('.manager-color').css('background-color', params.color);
    $('#manager-color__text').html(params.color);
    $('#manager-color__text').css('color', "#ffff");

}

function initEvents() {
    $('#reference_type_id').on('change', function () {
        setColorTypeReference();
