@php
    $nameElement='human-resources-permission-type';
    $nameProcess='HumanResourcesPermissionType';

@endphp
<script type="text/x-template" id="manager-dashboard-template">
    <div>
        <div class="panel panel-default">
            <div class="panel-heading"><strong>📅 Filtrar por Rango de Fechas</strong></div>
            <div class="panel-body">
                <form class="form-inline">
                    <div class="form-group">
                        <label for="startDate">Desde:</label>
                        <input type="date" class="form-control" id="startDate" placeholder="YYYY-MM-DD">
                    </div>
                    <div class="form-group" style="margin-left: 15px;">
                        <label for="endDate">Hasta:</label>
                        <input type="date" class="form-control" id="endDate" placeholder="YYYY-MM-DD">
                    </div>
                    <button type="button" class="btn btn-primary" id="applyFilters" style="margin-left: 15px;">
                        Aplicar Filtros
                    </button>
                </form>
            </div>
        </div>
        <div class="report-tracking-web">

            <div class="chart-container">
                <h2>1️⃣ Interacciones por Ubicación</h2>
                <div id="chart-location"></div>
            </div>

            <div class="chart-container">
                <h2>2️⃣ Fuentes de Tráfico por Empresa</h2>
                <div id="chart-sources"></div>
            </div>

            <div class="chart-container">
                <h2>3️⃣ Interacciones Diarias (Meetclic vs Externas)</h2>
                <div id="chart-daily"></div>
            </div>

            <div class="chart-container">
                <h2>4️⃣ Participación de Usuarios</h2>
                <div id="chart-users"></div>
            </div>

            <div class="chart-container">
                <h2>5️⃣ Tipos de Clic por Empresa</h2>
                <div id="chart-clicks"></div>
            </div>

            <div class="report-tracking-web__row">
                <div class="report-tracking-web__title"><?php echo '{{managerTracking.web.title}}' ?></div>
                <div class="report-tracking-web__data">

                </div>
            </div>
            <div class="report-tracking-web__row">
                <div class="report-tracking-web__title"><?php echo '{{managerTracking.business.title}}' ?></div>
                <div class="report-tracking-web__data">

                </div>
            </div>
        </div>


        <div class="report">
            <h1><?php echo '{{processName}}' ?></h1>
        </div>

        <div id="container-annotation"></div>

        <div id="container-line-labels"></div>

        <figure class="highcharts-figure">
            <div id="parent-container">
                <div id="play-controls">
                    <button id="play-pause-button" class="fa fa-play" title="play"></button>
                    <input id="play-range" type="range" value="1973" min="1973" max="2022"/>
                </div>
                <div id="container"></div>
            </div>
        </figure>
        <pre id="csv" style="display:none">
# Data from RIAA
year	LpEp	Vinyl	Cassette	CD	Downloads	Streams
1973	1.2	0.19	0.076
1974	1.4	0.194	0.087
1975	1.5	0.212	0.099
1976	1.7	0.245	0.146
1977	2.2	0.245	0.25
1978	2.5	0.26	0.45
1979	2.1	0.354	0.581
1980	2.2	0.25	0.705
1981	2.3	0.256	1.1
1982	1.9	0.283	1.4	0
1983	1.7	0.269	1.8	0.017
1984	1.5	0.299	2.4	0.103
1985	1.3	0.281	2.4	0.39
1986	0.983	0.228	2.5	0.93
1987	0.793	0.203	3	1.6
1988	0.532	0.18	3.4	2.1
1989	0.22	0.116	3.3	2.6
1990	0.086	0.094	3.5	3.5
1991	0.029	0.064	3	4.3
1992	0.014	0.066	3.1	5.3
1993	0.011	0.051	2.9	6.5
1994	0.018	0.047	3	8.5
1995	0.025	0.047	2.3	9.4
1996	0.036	0.048	1.9	9.9
1997	0.033	0	1.5	9.9
1998	0.034		1.4	11.4
1999	0.032		1.1	12.8
2000	0.028		0.626	13.2
2001	0.027		0.363	12.9
2002	0.021		0.21	12
2003	0.022		0.108	11.2	0
2004	0.019		0.023	11.4	0.138	0
2005	0.014			10.5	0.921	0.149
2006	0.016			9.4	1.631	0.206
2007	0.023			7.5	2.408	0.234
2008	0.057			5.5	2.612	0.321
2009	0.064			4.3	2.647	0.362
2010	0.089			3.4	2.62	0.461
2011	0.119			3.1	2.876	0.654
2012	0.161			2.5	2.946	1.033
2013	0.211			2.1	2.898	1.454
2014	0.244			1.8	2.5	1.827
2015	0.333			1.4	2.3	2.375
2016	0.355			1.1	1.769	3.818
2017	0.389			1	1.318	5.459
2018	0.419			0.696	0.945	7.255
2019	0.48			0.931	0.775	8.853
2020	0.644			0.483	0.622	10.078
2021	1			0.584	0.538	12.509
</pre>

        <div id="container-are-race-chart"></div>
    </div>

</script>
