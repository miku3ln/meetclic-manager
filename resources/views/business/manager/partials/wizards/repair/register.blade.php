<div id="content-data-rows content-data-rows--invoice">
    <div class="row">
        <div class=" col-md-4 col-sm-12">
            <div class="form-group" ng-class="getErrorForm('status')">
                <label for="status">Estado Mantenimiento</label>
                <select ng-model="model.attributes.status" name='status' class="form-control" required>
                    <?php echo '<option ng-repeat="x in statusData" value="{{x.id}}">{{x.text}}</option>'; ?>
                </select>
                <span class="messages"
                      ng-show="formManager.$submitted || formManager.status.$touched">
                        <span ng-show="formManager.status.$error.required" class="required ">Campo Requerido.</span>

                    </span>
            </div>
        </div>
        <div class=" col-md-4 col-sm-12">
            <div class="form-group" ng-class="getErrorForm('type_invoice')">
                <label for="type_invoice">Emitir</label>
                <select ng-change="_resultsTotal()" ng-model="model.attributes.type_invoice" name='type_invoice'
                        class="form-control" required>
                    <?php echo '<option ng-repeat="x in typeInvoiceData" value="{{x.id}}">{{x.text}}</option>'; ?>
                </select>
                <span class="messages"
                      ng-show="formManager.$submitted || formManager.type_invoice.$touched">
                        <span ng-show="formManager.type_invoice.$error.required"
