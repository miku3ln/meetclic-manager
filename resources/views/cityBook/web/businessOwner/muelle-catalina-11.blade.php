    ...
    * ============================== */
        ...
        const UI = (() => {
            const $loading = function () {
                return document.getElementById("ar-loading");
            };
            ...
            const container = function () {
                return document.querySelector(".container--custom");
            };
            const $reticle = function () {
                return document.getElementById("reticle-overlay");
            };
            const $map = function () {
                return document.getElementById("map");
            };
            const $btnBack = function () {
                return document.getElementById("btn-back-map");
            };
        ...
        class ModelViewerController {
            ...
            async setSource({glbUrl, usdzUrl}) {
                ...
                await new Promise((res, rej) => {
                    const onLoad = () => {
                        this.mv.removeEventListener('load', onLoad);
                        res();
                    };
                    const onErr = () => {
                        this.mv.removeEventListener('error', onErr);
                        rej();
                    };
            ...
            async activateARIfAny() {
                try {
                    if (typeof this.mv.activateAR === "function") this.mv.activateAR();
                } catch {
                }
            }
        ...
        class AndroidWebXRController {
            ...
            // Se debe llamar DENTRO del tap a la retícula (user activation)
                    ...
                    loader.load(glbUrl, (gltf) => {
                        ...
                        this.model.traverse(o => {
                            if (o.isMesh) {
                                ...
                                const m = o.material;
                                if (m) {
                                    Array.isArray(m) ? m.forEach(mm => mm.side = THREE.DoubleSide) : m.side = THREE.DoubleSide;
                                }
                            }
                        });
                        UI.hideLoading();
        ...
        const Viewer = (() => {
            ...
            async function onMarkerSourceSelected(glbUrl) {
                ...
                // Si ya hay sesión activa (AR o fallback), solo actualiza la fuente
                if (isActive()) {
                ...
                // Si el dispositivo soporta AR WebXR (Android), gate por retícula (user activation)
                if (await canUseAR()) {
                ...
                // --- Fallback inmediato (Web/iOS): crear <model-viewer> y asignar controller ---
                UI.revealContainer();
                UI.hideMap();
                UI.showFallback();
                ...
                controller = mvCtrl;                                   // <- controller ya NO es null
            ...
            async function handleReticleTap() {
                ...
                if (await canUseAR()) {
                    try {
                        ...
                        mode = "android-webxr";
                        controller.placeInFront();         // coloca al frente (sin hit-test)
                        return;
                    } catch (e) {
                        // cae a fallback si falla
                    }
                }

                // Fallback de respaldo (si no hay WebXR o falló)
                UI.showFallback();
                const mvCtrl = new ModelViewerController(UI.$mv(), {
                    onEnter: ({mode}) => UI.setHint(`AR activo (${mode}).`)
                });
            ...
            return {handleReticleTap, onMarkerSourceSelected, destroy, isActive};
        })();
    ...
    <footer class="popup-card__footer">
      ...
      <a class="popup-card__btn popup-card__btn--ghost"
        ...
        function initMap() {
            ...
            // Click libre: agrega marcador ad hoc
            map.on('click', (e) => {
