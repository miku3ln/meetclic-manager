...
@section('additional-scripts')
    ...
    <script>
        /* =========================================================
         * AR + Fallback con gestión de UI, eventos WebXR,
         * torch, tracking por poca luz, captura, retícula 2 pasos,
         * controles táctiles (rotate/scale/pan) y anti-rastro.
         * ========================================================= */
        ...
        class AndroidWebXRController {
            ...
            async _loadModel(glbUrl) {
                ...
                await new Promise((res, rej) => {
                    new THREE.GLTFLoader().load(glbUrl, (gltf) => {
                        ...
                        UI.hideLoading();
                        UI.setHint("Cámara lista. Usa la retícula para colocar.");
                        res();
                    }, undefined, (err) => {
                        UI.hideLoading();
            ...
            /* ====== Controles táctiles: rotar / pellizcar (escala) / dos dedos (mover) ====== */
            ...
            _onXRFrame(time, frame) {
                ...
                if (!pose) {
                    ...
                    return; // sin pose, no se presenta frame
                }
      ...
      <article class="popup-card" data-popup-id="${item.id}">
        ...
        class ViewerOrchestrator {
            constructor() {
                this.state = {mode: null, controller: null, pendingSource: null, arState: 'idle', lastSource: null};
            }
            ...
            async onMarkerSourceSelected(glbUrl) {
                ...
                this.state.lastSource = {glb: glbUrl, usdz: usdzUrl};

                if (this.isActive()) {
                    if (this.state.mode === 'android-webxr') {
                        await this.state.controller.setSource({glbUrl});
                        UI.setHint('Modelo actualizado (AR).');
                    } else {
                        await this.state.controller.setSource({glbUrl, usdzUrl});
                ...
                if (await Platform.canUseAR()) {
                    this.state.pendingSource = {glbUrl, usdzUrl};
                    this.state.arState = 'awaitSession';
                    UI.revealContainer();
                    UI.hideMap();
                    UI.hideFallback();
                    UI.setReticleText('Toca para iniciar la cámara');
                    UI.showReticle();
                    UI.setHint('Preparado para AR. Toca la retícula para abrir la cámara.');
                    return;
                }
                ...
                mvCtrl.bindOnce();
                await mvCtrl.setSource({glbUrl, usdz: usdzUrl});
                this.state.controller = mvCtrl;
                this.state.mode = Platform.isIOS ? 'ios-quicklook' : 'web-fallback';
                UI.showCapture();
                UI.setHint('Modelo listo en visor 3D.');
                this.state.arState = 'idle';
            }

            async handleReticleTap() {
                if (this.state.arState === 'awaitSession' && this.state.pendingSource) {
                    const {glbUrl, usdzUrl} = this.state.pendingSource;
                    try {
                        this.state.controller = new AndroidWebXRController({
                            onEnter: ({mode}) => {
                                UI.hideFallback();
                                UI.setHint(`Cámara iniciada (${mode}).`);
                            },
                            onExit: ({reason, state, status}) => {
                                UI.setHint(`Sesión finalizada (${reason || status || state || "desconocido"}).`);
                            }
                        });
                        await this.state.controller.startFromGesture(glbUrl);
                        this.state.mode = 'android-webxr';
                        this.state.arState = 'sessionReady';
                        UI.showCapture();
                ...
                if (this.state.arState === 'sessionReady' && this.state.mode === 'android-webxr') {
                    this.state.controller.placeInFront();
                    UI.hideReticle();
                    this.state.arState = 'idle';
                }
            }
            ...
            async destroy() {
                if (this.state.controller) {
                    if (this.state.mode === 'android-webxr') await this.state.controller.stop(); else this.state.controller.destroy();
                }
            ...
            async capture() {
                ...
                if (navigator.mediaDevices?.getDisplayMedia) {
                    try {
                        ...
                        console.warn('getDisplayMedia denegado/falló', e);
                    }
                }

                if (this.state.mode === 'android-webxr' && typeof this.state.controller.getCanvas === 'function') {
                    const cnv = this.state.controller.getCanvas();
                    if (cnv) {
                        DownloadUtils.saveDataURL(`captura-ar-${ts}.png`, cnv.toDataURL('image/png'));
                        const stats = this.state.controller.getModelStats?.() || {};
                        DownloadUtils.saveBlob(`captura-ar-${ts}.json`, new Blob([JSON.stringify({
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            // Volver al mapa (oculta TODOS los botones de gestión)
