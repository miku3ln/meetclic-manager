...
@section('additional-scripts')
    ...
    <script>
        ...
        /* ==============================
         * Android WebXR (sin hit-test) — controlador de sesión AR
         * - Inicia WebXR dentro del gesto del usuario (startFromGesture)
         * - Muestra “Abriendo cámara…” mientras calienta (grace period)
         * - Retícula sólo cuando hay primer pose válido
         * - Aviso por oscuridad + botón linterna (Torch)
         * - Estimación de luz (si el dispositivo la soporta)
         * - Gestos táctiles: rotar, pellizcar para escalar, dos dedos para mover
         * - Limpieza completa de sesión y recursos
         * - Utilidades: getCanvas() / getModelStats()
         * Requiere: THREE, UI (helper), Torch, StatsUtils
         * ============================== */
        class AndroidWebXRController {
            constructor(hooks = {}) {
                ...
                this._graceMs        = 1800; // tiempo para que encienda cámara/tracking
                this._noPoseWarnMs   = 900;  // umbral para avisar “oscuro” tras la gracia

                // Iluminación / estimación
                this._headlamp  = null; // luz que viaja con la cámara (mejora visibilidad)
                this._lightProbe = null;

                // Handlers de eventos XR (para poder quitarlos)
            ...
            // Debe llamarse DENTRO del click del usuario (user activation)
            async startFromGesture(glbUrl){
                // 1) requestSession PRIMERO (no romper user activation)
                ...
                // 2) reset de estado de tracking
                this._sessionStartMs = performance.now();
                this._firstPoseSeen  = false;
                this._noPoseSinceMs  = this._sessionStartMs;

                // 3) montar renderer/escena/cámara y enlazar sesión XR
                this._setupRenderer();
                this._setupScene();
                ...
                // 4) eventos de sesión XR
                this._onEnd = ()=> this.hooks.onExit && this.hooks.onExit({ reason:"session-end" });
                this._onVis = ()=> {
                    const st = this.session?.visibilityState;
                    if (st === "hidden" || st === "visible-blurred") {
                        this.hooks.onExit && this.hooks.onExit({ reason:"visibility", state: st });
                    }
                };
                ...
                // 6) cargar modelo (asíncrono)
                await this._loadModel(glbUrl);

                // 7) feedback de arranque
                UI.setHint("Abriendo cámara...");

                // 8) gestos táctiles (rotar/escala/mover)
                this._bindGestures();

                // 9) resize y callback de entrada
            ...
            _setupRenderer(){
                ...
                this._handleResize();
                this.renderer.setClearAlpha(0); // ver cámara atrás
            ...
            _setupScene(){
                ...
                // Luz base + direccional + “headlamp” para lugares oscuros
            ...
            async _loadModel(glbUrl){
                ...
                await new Promise((res, rej)=>{
                    ...
                    loader.load(glbUrl, (gltf)=>{
                        ...
                        // Normalizar escala ≈ 1 m de tamaño máximo
                        ...
                        this.model.traverse(o=>{
                            if (o.isMesh){
                                ...
                                const m = o.material;
                                (Array.isArray(m) ? m : [m]).forEach(mm => { if(mm){ mm.side=THREE.DoubleSide; mm.needsUpdate=true; } });
                            }
                        });
            ...
            /* ---------- gestos táctiles (rotate / pinch / two-finger pan) ---------- */
            ...
            _bindGestures(){
                ...
                const onStart = (e)=>{
                    ...
                    if (st.touches.length===1){ st.mode='rotate'; st.lastX=st.touches[0].clientX; st.lastY=st.touches[0].clientY; }
                    else if (st.touches.length>=2){
                        st.mode='pinch-pan';
                        const [a,b]=st.touches; st.lastDist=Math.hypot(a.clientX-b.clientX, a.clientY-b.clientY);
                        st.lastCx=(a.clientX+b.clientX)*0.5; st.lastCy=(a.clientY+b.clientY)*0.5;
                    }
                };
                ...
                const onMove = (e)=>{
                    ...
                    if (st.mode==='rotate' && st.touches.length===1){
                        const t=st.touches[0]; const dx=t.clientX-st.lastX;
                        this.model.rotation.y -= dx*0.01;
                        st.lastX=t.clientX; st.lastY=t.clientY; return;
                    }
                    ...
                    if (st.mode==='pinch-pan' && st.touches.length>=2){
                        ...
                        // mover en plano de cámara (dos dedos)
                        const dx=cx-st.lastCx, dy=cy-st.lastCy; st.lastCx=cx; st.lastCy=cy;
                ...
                const onEnd = ()=>{ st.touches=[]; st.mode='none'; };
            ...
            _onXRFrame(time, frame){
                ...
                // Aún sin pose (cámara calentando o tracking perdido)
                if (!pose) {
                    // periodo de gracia: no alarmar al usuario todavía
                    if (now - this._sessionStartMs < this._graceMs) {
                        UI.setHint("Abriendo cámara...");
                        return;
                    }
                    // fuera de la gracia: avisar si persiste
                    if (now - this._noPoseSinceMs > this._noPoseWarnMs) {
                        UI.setHint("Muy oscuro o sin textura. Aumenta la luz o activa la linterna.");
                        UI.showTorch();
                        this._noPoseSinceMs = now; // evita spam
                    }
                    return;
                }

                // Primer pose válido => mostrar retícula para colocar
            ...
            async _disposeModel(){
                ...
                this.model.traverse(o=>{
                    if (o.isMesh){
                        o.geometry?.dispose?.();
                        const m=o.material; Array.isArray(m) ? m.forEach(mm=>mm?.dispose?.()) : m?.dispose?.();
                    }
                });
                this.model=null;
            }
            ...
            getModelStats(){ return StatsUtils.computeModelStats(this.model); }
        }
