 ...
 * =========================================================== */
        ...
        const UI = (() => {
            ...
            return {
                ...
                resetLoadingProgress(label = 'Cargando modelo… 0%') {
                    $.loadingLbl && ($.loadingLbl.textContent = label);
                    $.loadingPct && ($.loadingPct.textContent = '0%');
                },
                updateLoadingProgress(p) {
                    ...
                    $.loadingLbl && ($.loadingLbl.textContent = `Cargando modelo… ${t}`);
                },
        ...
        class AndroidWebXRController {
            ...
            async startSessionFromGesture() {
                ...
                // light-probe
                try {
                    if (this.session.requestLightProbe) this._lightProbe = await this.session.requestLightProbe({type: 'spherical-harmonics'});
                } catch {
                }
        ...
        class ViewerOrchestrator {
            ...
            async onMarkerSourceSelected(glbUrl) {
                this._state.lastSource = {
                    glb: glbUrl,
                    usdz: glbUrl?.endsWith?.('.glb') ? glbUrl.replace(/\.glb$/i, '.usdz') : ''
                };
            ...
            async capture() {
                ...
                if (supportsTab) {
                    try {
                        ...
                        try {
                            ...
                            UI.setHint('✅ Captura (cámara + modelo).');
                            return;
                        } catch {
                        ...
                        console.warn('getDisplayMedia falló/denegado', e);
                    }
                }
                ...
                if (this._state.mode === 'android-webxr' && this._state.controller?.getCanvas) {
                    ...
                    save(blob);
                    UI.setHint('📸 Captura (solo modelo 3D).');
                    return;
                }

                if (UI.mv && window.html2canvas) {
                    const cnv = await window.html2canvas(UI.$fallback || UI.mv, {
                        useCORS: true,
                        backgroundColor: '#000'
                    });
                    const blob = await new Promise(res => cnv.toBlob(res, 'image/png'));
                    save(blob);
                    UI.setHint('📸 Captura (fallback 3D).');
                    return;
                }
