 ...
 * =========================================================== */
        ...
        class AndroidWebXRController {
            async captureWithVideoTextureQuad({
                ...
                // Guardar pose del modelo por seguridad
                const hadModel = !!this.model;
                const saved = hadModel ? {
                    pos: this.model.position.clone(),
                    quat: this.model.quaternion.clone(),
                    scl: this.model.scale.clone()
                } : null;

                // 1) Pausar XR pero conservar escena/renderer
                try {
                    if (typeof this.endXRButKeepScene === 'function') {
                        await this.endXRButKeepScene();
                    } else {
                        // --- Implementación mínima opcional ---
                        // if (this.session) { try { await this.session.end(); } catch {} this.session = null; }
                ...
                // 2) Iniciar cámara (getUserMedia + <video>)
                let stream = null, video = null, videoTex = null;
                try {
                    ...
                    Object.assign(video.style, {
                        ...
                        opacity: '0', pointerEvents: 'none', zIndex: '-1', left: '0', top: '0'
                    });
                    document.body.appendChild(video);

                    // Esperar metadatos + playing (para tener dimensiones reales)
                    await new Promise((res, rej) => {
                        const onMeta = () => {
                            cleanup();
                            res();
                    ...
                    await new Promise(r => setTimeout(r, 120)); // estabiliza un pelo

                    const vw = Math.max(1, video.videoWidth || 1280);
                    const vh = Math.max(1, video.videoHeight || 720);

                    // 3) Configurar renderer para el tamaño del video
                    ...
                    // 4) Crear VideoTexture
                    videoTex = new THREE.VideoTexture(video);
                    videoTex.minFilter = THREE.LinearFilter;
                    videoTex.magFilter = THREE.LinearFilter;
                    videoTex.generateMipmaps = false;
                    if (THREE.SRGBColorSpace) videoTex.colorSpace = THREE.SRGBColorSpace; // r150+
                    // videoTex.flipY = true; // (por defecto true) ajusta si ves la cámara invertida

                    // 5) Escena de fondo (quad a pantalla completa) + cámara ortográfica
                    ...
                    quad.renderOrder = -1000; // bien al fondo
                    bgScene.add(quad);

                    // 6) Render compuesto (1) cámara, (2) tu escena 3D
                    this.renderer.clear();
                    this.renderer.render(bgScene, bgCam);
                    // limpiar el z-buffer para que el 3D no quede tapado por el quad del fondo
                    this.renderer.clearDepth();
                    this.renderer.render(this.scene, this.camera);

                    // 7) Asegurar flush antes de leer el canvas
                    await new Promise(r => requestAnimationFrame(r));

                    // 8) Obtener blob del canvas del renderer
                    const blob = await new Promise(res => this.renderer.domElement.toBlob(res, type, quality));
                    ...
                    // 10) Reanudar XR y restaurar modelo exactamente como estaba
                    try {
                        if (typeof this.resumeXRSession === 'function') {
                            await this.resumeXRSession();
                        } else {
                            // --- Implementación mínima opcional ---
                            // this.renderer.xr.enabled = true;
                            // this.renderer.setAnimationLoop(this._loop);
                        }
                        if (typeof this.afterResumeXR === 'function') {
                            await this.afterResumeXR(); // “kick” para evitar que quede congelado
                        }
                    } catch {
                    }

                    if (hadModel && saved) {
                        this.model.position.copy(saved.pos);
                        this.model.quaternion.copy(saved.quat);
                        this.model.scale.copy(saved.scl);
                    }
                }
            }

// Guarda distancia cam-modelo y re-enciende XR; recoloca el modelo a la misma distancia
            async restartXRAfterCamera({rePlaceModel = true} = {}) {
                // 1) Calcula y guarda la distancia actual del modelo a la cámara (si hay modelo)
                let prevDist = null;
                try {
                ...
                // 2) Crear una nueva sesión XR y re-enganchar todo
                await this._resumeXRSessionInternal();

                // 3) “Kick” para que no se quede estático
                await this.afterResumeXR?.();

                // 4) Recolocar el modelo a la misma distancia frente a la cámara (opcional)
                if (rePlaceModel && this.model && this.camera && typeof prevDist === 'number' && isFinite(prevDist) && prevDist > 0.05) {
                    try {
                        const fwd = new THREE.Vector3(0, 0, -1).applyQuaternion(this.camera.quaternion).normalize();
                        const pos = new THREE.Vector3().copy(this.camera.position).addScaledVector(fwd, prevDist);
...
// Crea una sesión XR nueva, setea renderer.xr, referenceSpace y vuelve a poner listeners/lightProbe
            async _resumeXRSessionInternal() {
                // Por si ya hay una colgando
                try {
                    this.renderer?.setAnimationLoop(null);
                } catch {
                }
                ...
                // Re-crear la “promesa de primer frame” si la usas para UI/retícula
...
// === Dentro de ViewerOrchestrator ===
            async captureScreenFrame({
                                         type = 'image/jpeg',   // 'image/png' si quieres sin compresión
                                         quality = 0.95,
                                         download = true,
                                         filename
                                     } = {}) {
                ...
                // Para restaurar XR si el SO detiene/suspende la sesión
                ...
                try {
                    ...
                    // 1) Pedir captura de pantalla (usuario elige "Pantalla completa")
                    stream = await navigator.mediaDevices.getDisplayMedia({
                        video: {frameRate: 30}, audio: false
                    });

                    // 2) Montar <video> para leer un frame
                    video = document.createElement('video');
                    video.playsInline = true;
                    video.muted = true;
                    video.autoplay = true;
                    ...
                    // 3) Dibujar un frame en un canvas
                    ...
                    // 4) Convertir a Blob y (opcional) descargar
                    ...
                    // 6) Si XR estaba activo y la sesión se cayó, reanudar y restaurar pose
