     ...
     * ============================================================ */
        ...
        class GestorEventosModelo {
            ...
            constructor(viewer, opciones = {}) {
                ...
                this._abortCanvas = null;
                this._abortModelViewer = null;
            }
            detachAllListeners() {
                try { this._abortCanvas?.abort(); } catch {}
                try { this._abortModelViewer?.abort(); } catch {}
                this._escuchandoCanvas = false;
                this._escuchandoModelViewer = false;
        ...
        class JQueryArViewer {
            ...
            _unbindUi() {
                ...
                // Quita solo los handlers que agregamos (namespace .jqAr)
                $(ui.enterBtn).off('.jqAr');
                $(ui.resetBtn).off('.jqAr');
                $(ui.zoomInBtn).off('.jqAr');
                $(ui.zoomOutBtn).off('.jqAr');
                $(ui.scale1xBtn).off('.jqAr');
                $(ui.scale2xBtn).off('.jqAr');
                $(ui.rotLeftBtn).off('.jqAr');
                $(ui.rotRightBtn).off('.jqAr');
                $(ui.rotUpBtn).off('.jqAr');
                $(ui.rotDownBtn).off('.jqAr');
                $(ui.exploreBtn).off('.jqAr');
                $(ui.newPosBtn).off('.jqAr');
                this._uiBound = false;
            }
            async decideMode() {
...
// ➜ NUEVO: método público de destrucción total
            async destroy() {
                try {
                    // 1) termina la sesión XR si está activa
                    if (this.session) {
                        try { await this.session.end(); } catch {}
                    }

                    // 2) corta listeners del gestor de eventos (canvas/model-viewer)
                    try { this.eventos?.detachAllListeners?.(); } catch {}

                    // 3) desbind de UI jQuery
                    this._unbindUi();

                    // 4) quita handlers globales/resize y bucle de animación
                    try { this.$win?.off('resize'); } catch {}
                    try { this.renderer?.setAnimationLoop?.(null); } catch {}

                    // 5) limpia escena/modelo/canvas
                    try { this.resetEverything(); } catch {}
                    // resetEverything ya:
                    // - detiene animación
                    // - remueve modelo/escena
                    // - remueve canvas del DOM
                    // - limpia referencias internas

                    // 6) borra hint y estado visual opcional
                    try { $(this.cfg.ui.hint).text(''); } catch {}
                } finally {
                    // 7) por si acaso, nulifica referencias críticas
                    this.renderer = null;
                    this.scene = null;
                    this.camera = null;
                    this.session = null;
        ...
        let managerViewer;
        function initViewAr(params){
            let glbUrl=params["glbUrl"];
            managerViewer = new JQueryArViewer({
                glbUrl:glbUrl ,
                events: {
                    ...
                    onModelClick: ({modo, punto}) => {
                        ...
                        managerViewer.setStatus(`Click sobre modelo (${modo}) en ${p}`);
                    },
                    onDragStart: () => managerViewer.setStatus('Arrastre iniciado'),
                    onDrag: ({dx, dy}) => managerViewer.setStatus(`Arrastrando dx=${dx} dy=${dy}`),
                    onDragEnd: () => managerViewer.setStatus('Arrastre finalizado'),
                    onScale: ({scale}) => managerViewer.setStatus(`Escala: ${scale.toFixed(2)}`),
                    onRotate: ({rotY, rotX}) => managerViewer.setStatus(`Rotación Y=${rotY.toFixed(2)} X=${rotX.toFixed(2)}`)
                },
                i18n: {textos: {etiquetas: {botonVer: 'Entrar'}}}
            });
            window.viewer = managerViewer; // útil para depurar en consola
            managerViewer.init();
            // Ajustes recomendados
            managerViewer.configureCamera({toneExposure: 1.4});
            managerViewer.configureSensitivity({
                stableFramesRequired: 3,
                offsetRayDown: 0.25,
                upDotMin: 0.78,
                minDistance: 0.15
            });
            managerViewer.configureReticle({color: 0x00e5ff});

            // Política: colocar solo si el tap cae en el anillo de la retícula
            managerViewer.placeOnlyWhenReticleHit = true;

            // Mostrar contenedor tras verificación rápida del modo
            (async function verifyMode() {
                const mode = await managerViewer.decideMode();
                console.log('mode:', mode);
  ...
  `;
        ...
        async function swapModelAndPlace(newGlbUrl) {
            // 1) Cambiar la URL
            managerViewer.cfg.glbUrl = newGlbUrl;

            // 2) Quitar modelo actual (mantiene sesión XR y UI)
            if (managerViewer.model) managerViewer.reset();

            // 3) Volver a modo colocación (retícula ON)
            managerViewer.enterPlacementMode();

            // 4) Esperar retícula estable y colocar
            await waitForReticleVisible(managerViewer);
            managerViewer.placeOrMoveModelAtReticle();
        }
        function waitForReticleVisible(viewer, timeoutMs = 8000, pollMs = 60) {
            return new Promise((res, rej) => {
                const t0 = performance.now();
                (function tick() {
                    const ok = viewer?.reticle?.visible && viewer?.uiState?.inPlacementMode;
                    if (ok) return res(true);
                    if (performance.now() - t0 > timeoutMs) return rej(new Error('Retícula no visible'));
                    setTimeout(tick, pollMs);
                })();
            });
        }
        async function reinitViewAr(params) {
            destroyViewAr();
            // forzamos un micro-turno al event loop por si había tasks pendientes
            await Promise.resolve();
            initViewAr(params);
        }

        function destroyViewAr() {
            if (managerViewer) {
                try { managerViewer.destroy?.(); } catch(e) { console.warn(e); }
            }
            managerViewer = null;
            window.viewer = null;
            // Opcional: ocultar contenedor hasta reinit
            $(".container--custom").addClass("not-view");
        }
    </script>
@endsection
@section('content')
