...
@section('additional-styles')
    <style>
        ...
        .container--custom.not-view { opacity: 1; }
        .manager-buttons { display:flex; flex-wrap:wrap; gap:.5rem; }
        .manager-buttons--view-control-cam .not-view { display:inline-block !important; }
        .manager-buttons__item.not-view { display:none; }
    </style>
    ...
    <script>
        ...
        class GestorEventosModelo {
            ...
            constructor(viewer, opciones = {}) {
                ...
                const porDefecto = {
                    ...
                    gestures: {
                        habilitar: true,
                        rotacionFactor: 0.015,     // rad por px horizontal
                        rotacionFactorX: 0.010,    // rad por px vertical
                        escalaMin: 0.2,
                        escalaMax: 10.0,
                        pinchFactor: 0.005
                    },
                ...
                // Callback por defecto si no se provee
            ...
            _instalarOyentesCanvas() {
                ...
                // Click puntual sobre modelo en modo no-XR
                const onPointer = (evt) => {
                ...
                const onPointerDown = (evt) => {
                    ...
                    if (hits.length > 0 && !this._drag.activo) {
                        ...
                        try { this.cfg.onDragStart && this.cfg.onDragStart({evt, hit: hits[0]}); } catch {}
                        if (this.cfg.autorrotacion?.pausaAlInteractuar && this._autorrotacionActiva) {
                            this.desactivarAutorrotacion();
                        }
                    }
                    ...
                    if (this._pinch.activo && (evt.pointerId === this._pinch.id1 || evt.pointerId === this._pinch.id2)) {
                        if (this._pinch.id1 != null && this._pinch.id2 != null &&
                            ...
                            if (this._pinch.startDist === 0) {
                                ...
                                try { this.cfg.onScale && this.cfg.onScale({scale: next}); } catch {}
                            }
                        }
                        return;
                    }
                ...
                const onPointerUpCancel = (evt) => {
                    ...
                    if (this._drag.activo && evt.pointerId === this._drag.pointerId) {
                        ...
                        try { this.cfg.onDragEnd && this.cfg.onDragEnd({evt}); } catch {}
                    }
                };
            ...
            setOnDragStart(fn) { if (typeof fn === 'function') this.cfg.onDragStart = fn; }
            setOnDrag(fn) { if (typeof fn === 'function') this.cfg.onDrag = fn; }
            setOnDragEnd(fn) { if (typeof fn === 'function') this.cfg.onDragEnd = fn; }
            setOnScale(fn) { if (typeof fn === 'function') this.cfg.onScale = fn; }
            setOnRotate(fn) { if (typeof fn === 'function') this.cfg.onRotate = fn; }
        }
        ...
        class JQueryArViewer {
            constructor(options = {}) {
                ...
                // NUEVO: exigir tap exacto sobre retícula
                this.placeOnlyWhenReticleHit = true;

                // Raycaster temporal para retícula
                this._tmpRaycaster = null;
            }
            ...
            async onSessionStarted(session) {
                ...
                try {
                    ...
                    if (downRay) {
                        hitSource = await session.requestHitTestSource({space: this.viewerSpace, offsetRay: downRay});
                    } else {
            ...
            // NUEVO: util para rayo de controlador/tap XR
            _getControllerRay() {
                const controller = this.renderer?.xr?.getController(0);
            ...
            // NUEVO: verificar si el rayo impacta el anillo de la retícula
            _isRayOnReticle(ray) {
                if (!this.reticle) return false;

                // Plano de la retícula en mundo
                const center = new THREE.Vector3();
                this.reticle.getWorldPosition(center);

                const normal = new THREE.Vector3(0, 1, 0)  // la retícula está girada a XZ
                ...
                // Distancia radial en XZ con respecto al centro de la retícula
                const dx = hitPoint.x - center.x;
                const dz = hitPoint.z - center.z;
                const r = Math.hypot(dx, dz);

                // Acepta TODO el disco hasta el radio externo (tolerancia 5 %)
                ...
                try {
                    ...
                    if (this.placeOnlyWhenReticleHit) {
                        ...
                        if (!ray || !this._isRayOnReticle(ray)) {
                            // Mensaje más exacto cuando hay retícula pero el tap no cayó encima
                            this.setStatus('Toca exactamente sobre la retícula para colocar.'); // puedes moverlo a i18n si deseas
                            return;
                        }
                    }

                    // Colocar o mover en la posición de la retícula
                    this.placeOrMoveModelAtReticle();

                    // Click por proximidad para gestos/callbacks
                    if (this.eventos && typeof this.eventos.onSelectXR === 'function') {
                        this.eventos.onSelectXR(ret.matrixWorld);
                    }
            ...
            applyMatrixToModel(model, mat4) {
                ...
                model.updateMatrixWorld(true); // asegura coherencia
            }
            ...
            _smoothReticleUpdate(targetMatrix) {
                if (!this._reticleState) {
                    ...
                    this.reticle.updateMatrixWorld(true); // ← IMPORTANTE
                    return;
                }
            ...
            initViewUICam() {
                ...
                $("#btn-reset,#btn-zoom-in,#btn-zoom-out,#btn-scale-1x,#btn-scale-2x,#btn-rot-left,#btn-rot-right,#btn-rot-up,#btn-rot-down").removeClass("not-view");
            }

            removeViewUICam() {
                $(".manager-buttons").removeClass("manager-buttons--view-control-cam");
                $("#btn-reset,#btn-zoom-in,#btn-zoom-out,#btn-scale-1x,#btn-scale-2x,#btn-rot-left,#btn-rot-right,#btn-rot-up,#btn-rot-down").addClass("not-view");
            }
    ...
    <div class="container--custom not-view">
        ...
        <div class="d-flex flex-wrap gap-2 mb-3">
            <div class="manager-buttons">
                ...
                <button id="btn-reset" class="btn btn-outline-secondary btn-sm manager-buttons__item not-view">Reset</button>
                <button id="btn-zoom-in" class="btn btn-outline-success btn-sm manager-buttons__item not-view">Zoom +</button>
                <button id="btn-zoom-out" class="btn btn-outline-success btn-sm manager-buttons__item not-view">Zoom −</button>
                <button id="btn-scale-1x" class="btn btn-outline-info btn-sm manager-buttons__item not-view">Scale 1×</button>
                <button id="btn-scale-2x" class="btn btn-outline-info btn-sm manager-buttons__item not-view">Scale 2×</button>
                <div class="vr"></div>
                <button id="btn-rot-left" class="btn btn-outline-warning btn-sm manager-buttons__item not-view">⟲ Left</button>
                <button id="btn-rot-right" class="btn btn-outline-warning btn-sm manager-buttons__item not-view">⟳ Right</button>
                <button id="btn-rot-up" class="btn btn-outline-warning btn-sm manager-buttons__item not-view">↑ Up</button>
                <button id="btn-rot-down" class="btn btn-outline-warning btn-sm manager-buttons__item not-view">↓ Down</button>
            </div>
        </div>
