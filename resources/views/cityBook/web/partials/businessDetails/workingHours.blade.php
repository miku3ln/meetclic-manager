                ...
                <ul>
                    ...
                    @foreach ($dataManagerPage['business']['scheduling'] as $schedule)
                        <li class="{{$schedule->currentClass}}">
                            @php
                                $allDaySet =$schedule->status=="Closed"? __('frontend.business-details.all-day-closed'): __('frontend.business-details.all-day-open');
                           $allDaySet.=" " .$schedule->schedulingType;

                            @endphp
                            @if($schedule->type==0)

                                <span class="opening-hours-day">{{$schedule->text}} </span>
                                <span class="opening-hours-time">{{$allDaySet}}
                                ...
                                <ul class="opening-hours-time__ul">
                                    ...
                                    @foreach ($schedule->breakdown as $breakdown)
                                        <li class="opening-hours-time__li">
                                            <span>{{$breakdown['start_time']['modelBreakdown']}}</span> a
                                            <span>{{$breakdown['end_time']['modelBreakdown']}}</span>
