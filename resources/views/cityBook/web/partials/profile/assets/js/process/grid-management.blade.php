<script id="grid-management">
    function formatDateEs(dateTime) {
        if (!dateTime) return '';
        ...
        return `${dayName}, ${day} ${month} ${year}, ${hours}:${minutes}:${seconds} ${period}`;
    }

    var componentThisBusiness;
    Vue.component('grid-manager-component', {
        template: '#grid-management-template',
        directives: {
            'init-grid-filters': {
...
//**Modal*
                ...
                gridConfig: {
                    selectorCurrent: "#account_gamification_by_movement-grid",
                    url: $("#action_accountGamificationByMovementAdmin_admin").val()
                },
                submitStatus: "no",
                showManager: false,
                managerType: null,
        ...
        methods: {
            ...
            _search: function (params) {
                ...
                $('#sort-by').on('change', function () {
                    ...
                    var nameKey = $(optionSelected).attr("id");
                    selectorCurrent = '#' + nameKey;
                    var sortCurrent = $(selectorCurrent).attr('order');
                    titleOption = $(optionSelected).attr("title");
                    $(selectorCurrent).attr('order-type');
                    if (sortCurrent == 'asc') {
                        if ($(selectorCurrent).attr('order-type') == null) {
                            titleOption += ' ASC';
                            sortCurrent = "asc";
                            $(selectorCurrent).attr('order-type', sortCurrent);
                        } else {
                            titleOption += ' DES';
                            sortCurrent = "desc";
                        }
                        $(selectorCurrent).attr('order', sortCurrent);
                    } else {
                        titleOption += ' DESC';
                    }
                ...
                gridInit.bootgrid({
                    ...
                    formatters: {
                        'description': function (column, row) {
                            ...
                            const formatted = formatDateEs(rawDate);
                            var classInputOutput = row.direction_default == "IN" ? "badge-points--success" : "badge-points--danger";
                            var textUpDown = row.direction_default == "IN" ? "+" : "-";
                            var informationDate = [
                                '<span class="badge__view-points badge__view-points--info">',
                                formatted,
                                '</span> <br>'
                            ];
                            ...
                            var descriptionInformation = [
                                '<div class="dashboard-list">',
                                '   <div class="dashboard-message"><span class="new-dashboard-item ' + classInputOutput + '"><i class="' + classInputOutputIconView + '"></i></span>',
                                '     <div class="dashboard-message-text">',
                                '         <p>',
                                '          <span class="badge badge--size-large ' + classInputOutput + '">' + textUpDown + row.amount +

                                '              <span class="dashboard-list__content-type-money">  <i class="fa' + (row.type_money == 0 ? " fa-star-o" : " fa-trophy") + ' "></i></span> ',
                                '         </span> ',
                                '        <span  class="dashboard-list__content-description-task">',
                                informationDate.join(""),
                                '                <span class="dashboard-list__content-task-type"> <i class="' + row.icon_class + '"></i></span> ',
                                row.description,
                                typeEntity.join(""),
                                '        </span>',
