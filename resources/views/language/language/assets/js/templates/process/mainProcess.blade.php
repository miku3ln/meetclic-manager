    ...
    <div>
        ...
        <div class='content-component'>
            ...
            <div class="content-form" v-if="showManager">
                <b-form id="languageForm" v-on:submit.prevent="_submitForm">
       ...
       <?php  echo "{{model.structure.value.maxLength.msj}}"?>
                            ...
                            <b-col md="6">
                                <div class="form-group"
                                     ...
                                     :class="getClassErrorForm('initials',$v.model.attributes.initials)">
                                    <label class="form__label " v-html='getLabelForm("initials")' ></label>
                                    <div class="content-element-form">
                                        <select v-model.trim="$v.model.attributes.initials.$model"
                                                v-bind:id="getNameAttribute('initials')"
                                                v-bind:name="getNameAttribute('initials')"
                                                class="form-control m-input"
                                                @change="_setValueForm('initials', $v.model.attributes.initials.$model)"
                                        >
                                            <option v-for="(row,index) in model.structure.initials.options"
                                                    v-bind:value="row.value"><?php echo '{{row.text}}' ?>
                                            </option>
                                        </select>
                                    </div>
                                    <div class="content-message-errors">
                                        <b-form-invalid-feedback
                                            :state="!$v.model.attributes.initials.$error">
      <span v-if="!$v.model.attributes.initials.required">
       <?php  echo "{{model.structure.initials.required.msj}}"?>
      </span>
                                            <span v-if="!$v.model.attributes.initials.maxLength">
       <?php  echo "{{model.structure.initials.maxLength.msj}}"?>
      </span>
                                        </b-form-invalid-feedback>
                                    </div>
                                </div>
