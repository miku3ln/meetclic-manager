...
># Acceso a Apache (HTTP/HTTPS) y MySQL desde Laravel
...
- **HTTPS (localhost):**  
  ...
  *(verás aviso por certificado auto-firmado; acepta la excepción para continuar)*

## Apache (desde otro dispositivo en tu red)

- **HTTP (LAN):**  
  `http://192.168.137.1:9080/`

- **HTTPS (LAN):**  
  `https://192.168.137.1:9443/`

## Ejemplos de proyecto (sin “/public” en la URL)

- `http://localhost:9080/meetclickmanager`
- `https://localhost:9443/meetclickmanager`

> **Nota:** si cambiaste puertos en tu `.env` del stack (`HTTP_PORT`, `HTTPS_PORT`), reemplaza `9080` / `9443` por los que uses.

---

## Conexión MySQL desde Laravel

### Opción A — Laravel corriendo **dentro del contenedor Apache**
Usa el **nombre del servicio** de MySQL en Docker (resolución por DNS interno):
